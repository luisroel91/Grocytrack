(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('immer-peasy'), require('react'), require('redux'), require('redux-thunk'), require('memoizerific')) :
  typeof define === 'function' && define.amd ? define(['exports', 'immer-peasy', 'react', 'redux', 'redux-thunk', 'memoizerific'], factory) :
  (global = global || self, factory(global.EasyPeasy = {}, global.produce, global.React, global.Redux, global.ReduxThunk, global.memoizerific));
}(this, function (exports, produce, React, redux, reduxThunk, memoizerific) { 'use strict';
  ...
  function createStoreStateHook(Context) {
    return function useStoreState(mapState, dependencies) {
      ...
      var isActive = React.useRef(true); // Tracks when a hooked component is unmounting

      var unmounted = React.useRef(false); // Throwing the error inline allows error boundaries the opportunity to
      // catch the error
  ...
  var actionStateSymbol = '🙈actionState🙈';
  var computedSymbol = '🙈computedSymbol🙈';
  var computedConfigSymbol = '🙈computedConfigSymbol🙈';
  ...
  var thunkStateSymbol = '🙈thunkState🙈';

  var actionName = function actionName(action) {
    return action[actionNameSymbol];
  };
  var debug = function debug(state) {
    if (produce.isDraft(state)) {
      var final = produce.finishDraft(produce.createDraft(state));
      return final;
    }
  ...
  function createStoreInternals(_ref) {
      ...
      if (parentPath.length > 0) {
        ...
        if (target) {
          if (!selector.prevState || selector.prevState !== get(parentPath, state)) {
            ...
            newState = produce__default(state, function (draft) {
    ...
    var createReducer = function createReducer() {
      var runActionReducerAtPath = function runActionReducerAtPath(state, action, actionReducer, path) {
        ...
        var updatedState = path.length === 0 ? produce__default(state, function (_draft) {
          return actionReducer(_draft, action.payload);
        }) : produce__default(state, function (draft) {
          set(actionReducer[metaSymbol].parent, draft, produce__default(current, function (_draft) {
            return actionReducer(_draft, action.payload);
          }));
        });
      ...
      var reducerForCustomReducers = function reducerForCustomReducers(state, action) {
        return produce__default(state, function (draft) {
  ...
  function createStore(model, options) {
    ...
    var store = redux.createStore(references.internals.reducer, references.internals.defaultState, composeEnhancers.apply(void 0, [redux.applyMiddleware.apply(void 0, [reduxThunk, dispatchActionStringListeners].concat(middleware, [listenerActionsMiddleware, currentStateMiddleware, mockActionsMiddleware]))].concat(enhancers)));
  ...
  exports.thunk = thunk;
  exports.thunkCompleteName = thunkCompleteName;
  exports.thunkFailName = thunkFailName;
  exports.thunkStartName = thunkStartName;
